...
// 対応型は bool を除く 64bit 以下の整数と double、スカラー版の 2 引数は同じ型を指定
...
private:
    ...
    // span にできる配列の要素型、コンテナ本体はコピーしない
    template<class R>
    using array_element = typename decltype(span{declval<R>()})::element_type;

    // 書き込み用が既定、読み取り用では const 要素も許可する
    template<class R, bool allow_const = false>
    static constexpr bool array_type = requires {
        typename array_element<R>;
        requires !is_volatile_v<array_element<R>>
              && (allow_const || !is_const_v<array_element<R>>)
              && (integer_type<array_element<R>> || same_as<remove_const_t<array_element<R>>, double>);
    };

    // 非空・非負・有限の重みを合計する、合計は正かつ有限が前提
    static double weight_sum(auto a) noexcept {
...
public:
    ...
    template<class R> requires (array_type<R> && same_as<array_element<R>, double>)
    inline void uniform(R&& out) noexcept {
        for (auto& x : span{out}) x = uniform();
    }

    // 配列を半開区間 [0, r) で埋める、端点は配列の要素型に変換される
    template<class R> requires array_type<R>
    ...
    inline void uniform(R&& out, array_element<R> l, array_element<R> r) noexcept {
        for (auto& x : span{out}) x = uniform(l, r);
    }

    // 整数配列を閉区間 [0, r] で埋める
    template<class R> requires (array_type<R> && integer_type<array_element<R>>)
    ...
    inline void uniform_closed(R&& out, array_element<R> l, array_element<R> r) noexcept {
        for (auto& x : span{out}) x = uniform_closed(l, r);
    }
    ...
    public:
        ...
        template<class R> requires array_type<R, true>
...
// 配列版の型制約、const span 自体は参照先が書き込み可能なら利用できる
// 引数の数と要素型を固定し、スカラー版との曖昧なオーバーロードを検出する
template<class R> concept HasUnitFill = requires(FastRng& rng, R&& a) { rng.uniform(forward<R>(a)); };
template<class R> concept HasRangeFill = requires(FastRng& rng, R&& a) {
    { rng.uniform(forward<R>(a), 10) } -> same_as<void>;
    { rng.uniform(forward<R>(a), 0, 10) } -> same_as<void>;
};
template<class R> concept HasClosedFill = requires(FastRng& rng, R&& a) {
    { rng.uniform_closed(forward<R>(a), 10) } -> same_as<void>;
    { rng.uniform_closed(forward<R>(a), 0, 10) } -> same_as<void>;
};
template<class R> concept HasPerm = requires(FastRng& rng, R&& a) {
    { rng.perm(forward<R>(a)) } -> same_as<void>;
    { rng.perm(forward<R>(a), 0) } -> same_as<void>;
};
static_assert(HasRangeFill<vector<int>&> && HasClosedFill<vector<int>&> && HasPerm<vector<int>&>);
static_assert(HasRangeFill<array<int, 4>&> && HasPerm<int(&)[4]>);
static_assert(HasRangeFill<span<int>> && HasRangeFill<span<int, 4>>);
static_assert(HasRangeFill<const span<int>&> && HasPerm<const span<int>&>);
static_assert(HasUnitFill<vector<double>&> && HasUnitFill<span<double, 0>>);
static_assert(!HasUnitFill<vector<int>&> && !HasClosedFill<vector<double>&> && !HasPerm<vector<double>&>);
static_assert(!HasRangeFill<const vector<int>&> && !HasRangeFill<span<const int>>);
static_assert(!HasRangeFill<array<const int, 4>&> && !HasRangeFill<span<volatile int>>);
static_assert(!HasPerm<vector<bool>&> && !HasRangeFill<vector<bool>&>);
static_assert(!HasRangeFill<vector<float>&> && !HasRangeFill<vector<__int128_t>&>);
static_assert(!HasRangeFill<vector<ScopedEnum>&> && !HasRangeFill<vector<Convertible>&>);
static_assert(!HasRangeFill<deque<int>&> && !HasRangeFill<list<int>&>);
static_assert(!HasPerm<int*> && !HasPerm<vector<int>> && !HasRangeFill<vector<int>>);
static_assert(noexcept(FastRng{}.perm(declval<vector<int>&>())));
static_assert(noexcept(FastRng{}.uniform(declval<vector<double>&>())));
static_assert(noexcept(FastRng{}.uniform_closed(declval<vector<int>&>(), 1, 6)));

// ADL でのみ走査できる連続範囲、メンバーの begin/end は関数ではなく添字演算子も持たない
// span が受け付ける型を、直接の range-for や out[i] への変更で取りこぼさないことを確認する
template<class T>
struct AdlArray {
    enum { begin = 0, end = 0 };
    T* first;
...
static_assert(ranges::contiguous_range<AdlArray<int>> && ranges::sized_range<AdlArray<int>>);
static_assert(HasRangeFill<AdlArray<int>&> && HasClosedFill<AdlArray<int>&> && HasPerm<AdlArray<int>&>);
static_assert(HasUnitFill<AdlArray<double>&> && !HasPerm<AdlArray<double>&>);
static_assert(!HasRangeFill<AdlArray<const int>&> && !HasRangeFill<AdlArray<volatile int>&>);
...
void test_contiguous_array_views(Tester& t) {
    ...
    [&]<class... Ts>(tuple<Ts...>) {
        (test_contiguous_array_view<Ts>(t, [](auto* p, size_t n) { return AdlArray<Ts>{p, p + n}; }), ...);
        (test_contiguous_array_view<Ts>(t, [](auto* p, size_t n) {
            return ranges::subrange(counted_iterator{p, (ptrdiff_t)n}, default_sentinel);
        }), ...);
    }(tuple<int, uint64_t, double>{});
}
...
void check_fill(Tester& t, size_t n, uint64_t seed_value, Fill fill, Scalar scalar) {
    ...
    for (auto x : values) {
        ...
        if constexpr (same_as<T, double>) {
            t.require(bit_cast<uint64_t>(x) == bit_cast<uint64_t>(expected), "double fill scalar bit equality");
        } else {
...
void test_integer_arrays(Tester& t, mt19937_64& data) {
    ...
    // C 配列・array・固定長 span・動的 span・const な view で同じ出力になる
    T raw_array[17]{};
    array<T, 17> fixed{};
    auto copy = fixed;
    ...
    c.uniform(span{copy}, T{1}, T{7});
    auto view = span<T>{raw_array};
    ...
    fixed.fill(T{99});
    FastRng sub(77);
    sub.uniform_closed(span{fixed}.subspan(3, 8), T{1}, T{7});
    ...
    sub.uniform(span<T>{}, T{1});
    sub.uniform_closed(span{fixed}.last(0), T{0});
...
void test_double_arrays(Tester& t, mt19937_64& data) {
    ...
    a.fill(-99.0);
    first.uniform(span{a}.subspan<2, 5>(), 1.0);
...
void test_permutations(Tester& t, mt19937_64& data) {
    ...
    first.perm(a); second.perm(span{b}, T{0}); third.perm(c);
    ...
    a.fill(T{99});
    first.perm(span{a}.subspan(3, 8), T{10});
...
void test_array_apis(Tester& t, mt19937_64& data) {
    [&]<class... Ts>(tuple<Ts...>) {
        (test_integer_arrays<Ts>(t, data), ...);
        (test_permutations<Ts>(t, data), ...);
...
template<class R> concept HasWeighted = requires(FastRng& rng, R&& a, FastRng::WeightedTable& table) {
    { rng.weighted_index(forward<R>(a)) } -> same_as<int32_t>;
    FastRng::WeightedTable{forward<R>(a)};
    table.assign(forward<R>(a));
};
static_assert(HasWeighted<vector<int>&> && HasWeighted<const vector<double>&>);
static_assert(HasWeighted<const array<uint64_t, 4>&> && HasWeighted<const int(&)[4]>);
static_assert(HasWeighted<span<const double>> && HasWeighted<span<int, 4>>);
static_assert(HasWeighted<AdlArray<const double>&>);
static_assert(!HasWeighted<vector<bool>&> && !HasWeighted<vector<float>&>);
static_assert(!HasWeighted<vector<long double>&> && !HasWeighted<vector<__uint128_t>&>);
static_assert(!HasWeighted<vector<ScopedEnum>&> && !HasWeighted<vector<Convertible>&>);
static_assert(!HasWeighted<span<volatile double>> && !HasWeighted<deque<int>&>);
static_assert(!HasWeighted<int*> && !HasWeighted<int>);
static_assert(same_as<decltype(FastRng{}.weighted_index(declval<const FastRng::WeightedTable&>())), int32_t>);
static_assert(sizeof(FastRng) == sizeof(uint64_t));

// 独立した累積配列と upper_bound でワンショットの結果を照合する
int32_t reference_weighted(span<const double> weights, uint64_t raw) {
...
void test_weighted(Tester& t, mt19937_64& data) {
    ...
    [&]<class... Ts>(tuple<Ts...>) { (test_weight_type<Ts>(t), ...); }
        (tuple<signed char, unsigned char, short, unsigned short, int, unsigned int,
               long, unsigned long, long long, unsigned long long,
               char, wchar_t, char8_t, char16_t, char32_t, double>{});
...
// const span<double> は利用可能、span<const double> は利用不可
template<class R> concept HasUniformSum = requires(FastRng& rng, R&& a) {
    { rng.uniform_sum(forward<R>(a)) } -> same_as<void>;
    { rng.uniform_sum(forward<R>(a), 10.0) } -> same_as<void>;
};
static_assert(HasUniformSum<vector<double>&> && HasUniformSum<array<double, 4>&>);
static_assert(HasUniformSum<double(&)[4]> && HasUniformSum<span<double>> && HasUniformSum<span<double, 4>>);
static_assert(HasUniformSum<span<double, 0>> && HasUniformSum<const span<double>&>);
static_assert(HasUniformSum<AdlArray<double>&> && HasUniformSum<array<double, 0>&>);
static_assert(!HasUniformSum<const vector<double>&> && !HasUniformSum<span<const double>>);
static_assert(!HasUniformSum<array<const double, 4>&> && !HasUniformSum<span<volatile double>>);
static_assert(!HasUniformSum<vector<float>&> && !HasUniformSum<vector<long double>&>);
static_assert(!HasUniformSum<vector<int>&> && !HasUniformSum<vector<bool>&>);
static_assert(!HasUniformSum<deque<double>&> && !HasUniformSum<list<double>&>);
static_assert(!HasUniformSum<double*> && !HasUniformSum<vector<double>> && !HasUniformSum<int>);
static_assert(noexcept(FastRng{}.uniform_sum(declval<vector<double>&>())));

// 比較用の指数乱数正規化、3 要素にも一般の場合と同じ処理を行う
void sum_exponential(FastRng& rng, span<double> a, double s) {
    assert(s >= 0 && isfinite(s) && (!a.empty() || s == 0));
    if (a.size() <= 1 || s == 0) { fill(a.begin(), a.end(), s); return; }
...
void sum_sorted_cuts(FastRng& rng, span<double> a, double s) {
    if (a.size() <= 1 || s == 0) { fill(a.begin(), a.end(), s); return; }
...
void check_sum_values(Tester& t, span<const double> a, double s) {
    auto total = 0.0L;
    for (auto x : a) {
...
void check_sum_case(Tester& t, size_t n, double s, uint64_t seed_value) {
    ...
    auto a = span{storage}.subspan(2, n);
...
void test_uniform_sum(Tester& t, mt19937_64& data) {
    ...
    for (auto k = 0; k < 12000; ++k) {
        ...
        auto s = n == 0 ? 0.0 : bit_cast<double>(raw);
...
template<class R> concept HasNormalFill = requires(FastRng& rng, R&& a) {
    { rng.normal(forward<R>(a)) } -> same_as<void>;
    { rng.normal(forward<R>(a), 10.0) } -> same_as<void>;
    { rng.normal(forward<R>(a), 10.0, 2.0) } -> same_as<void>;
};
static_assert(same_as<decltype(FastRng{}.normal()), double>);
static_assert(same_as<decltype(FastRng{}.normal(10)), double>);
static_assert(noexcept(FastRng{}.normal()) && noexcept(FastRng{}.normal(declval<vector<double>&>())));
static_assert(HasNormalFill<vector<double>&> && HasNormalFill<array<double, 4>&>);
static_assert(HasNormalFill<double(&)[4]> && HasNormalFill<span<double>> && HasNormalFill<span<double, 4>>);
static_assert(HasNormalFill<const span<double>&> && HasNormalFill<array<double, 0>&>);
static_assert(HasNormalFill<span<double, 0>> && HasNormalFill<AdlArray<double>&>);
static_assert(!HasNormalFill<const vector<double>&> && !HasNormalFill<span<const double>>);
static_assert(!HasNormalFill<array<const double, 4>&> && !HasNormalFill<span<volatile double>>);
static_assert(!HasNormalFill<vector<float>&> && !HasNormalFill<vector<long double>&>);
static_assert(!HasNormalFill<vector<int>&> && !HasNormalFill<vector<bool>&>);
static_assert(!HasNormalFill<vector<__int128_t>&> && !HasNormalFill<vector<ScopedEnum>&>);
static_assert(!HasNormalFill<deque<double>&> && !HasNormalFill<list<double>&>);
static_assert(!HasNormalFill<double*> && !HasNormalFill<vector<double>> && !HasNormalFill<int>);
...
void check_normal_case(Tester& t, size_t n, double mean, double stddev, uint64_t seed_value) {
    ...
    if (stddev == 0) {
        for (auto x : a) t.require(bit_cast<uint64_t>(x) == bit_cast<uint64_t>(mean), "normal zero stddev preserves mean bits");
    } else {
...
void test_normal_distribution(Tester& t, size_t block, double mean, double stddev) {
    ...
    while (samples < count) {
        ...
        else rng.normal(span{buffer}.first(n), mean, stddev);
...
void test_normal(Tester& t, mt19937_64& data) {
    ...
    for (auto trial = 0; trial < 10000; ++trial) {
        ...
        for (auto i = 0; i < 8; ++i) {
            ...
            t.require(bit_cast<uint64_t>(x) == bit_cast<uint64_t>(twin.normal(mean, stddev)), "normal scalar copy reproducibility");
            if (stddev == 0) t.require(bit_cast<uint64_t>(x) == bit_cast<uint64_t>(mean), "normal scalar zero stddev");
    ...
    // 全ての配列入口・既定引数と、span を経由する連続範囲の互換性を検査する
    for (auto seed_value = uint64_t{0}; seed_value < 100; ++seed_value) {
        ...
        auto view = ranges::subrange(counted_iterator{c, 7}, default_sentinel);
        r3.normal(view, -2, 0);
    ...
    for (auto seed_value = uint64_t{0}; seed_value < 64; ++seed_value) {
        ...
        FastRng r1(seed_value), r2(seed_value), r3(seed_value);
        r1.normal(a); r2.normal(span{b}.first(16)); r2.normal(span{b}.subspan(16));
        t.require(a == b && r1.state == r2.state, "normal even split equivalence");
        r3.normal(span{c}.first(13)); r3.normal(span{c}.subspan(13));
...
[[gnu::noinline]] Sample measure_array(vector<T>& a, uint64_t arrays, uint64_t seed_value, F fn) {
    ...
    for (auto x : a) {
        if constexpr (same_as<T, double>) checksum = rotl(checksum, 1) ^ bit_cast<uint64_t>(x);
        else checksum = rotl(checksum, 1) ^ (uint64_t)x;
    }
    bench_sink = bench_sink ^ checksum;
    return {chrono::duration<double, milli>(end - start).count(), checksum};
}
