...
public:
    ...
    struct CsvStatHook {
        ...
        explicit CsvStatHook(std::string path = "chokudai_stat.csv", double interval_ms = 50.0)
            : filename(std::move(path)), interval_us(multi_detail::to_us(interval_ms)) {}
    ...
    requires StateArg<U, State> && StepArg<S>
    bool push(U&& state, Cost cost, S step) {
        auto maker = [&]() -> decltype(auto) { return std::forward<U>(state); };
        return engine_.push_candidate(turn_, step, cost, now_hash_, false, false, maker);
    }

    // 状態を候補として追加する、期待償却 O(log W + S)、到着キュー初回確保は O(W S)
    template<class U, class S, class B>
    requires StateArg<U, State> && StepArg<S> && BoolArg<B>
    bool push(U&& state, Cost cost, S step, B finished) {
        auto maker = [&]() -> decltype(auto) { return std::forward<U>(state); };
        return engine_.push_candidate(turn_, step, cost, now_hash_, false, finished, maker);
    }

    // 状態を候補として追加する、期待償却 O(log W + S)、到着キュー初回確保は O(W S)
    template<class U, class H, class S>
    requires StateArg<U, State> && CompatibleHashArg<H, Hash> && StepArg<S>
    bool push(U&& state, Cost cost, H hash, S step) {
        auto maker = [&]() -> decltype(auto) { return std::forward<U>(state); };
        return engine_.push_candidate(turn_, step, cost, static_cast<Hash>(hash), true, false, maker);
    }

    // 状態を候補として追加する、期待償却 O(log W + S)、到着キュー初回確保は O(W S)
    template<class U, class H, class S, class B>
    requires StateArg<U, State> && CompatibleHashArg<H, Hash> && StepArg<S> && BoolArg<B>
    bool push(U&& state, Cost cost, H hash, S step, B finished) {
        auto maker = [&]() -> decltype(auto) { return std::forward<U>(state); };
        return engine_.push_candidate(turn_, step, cost, static_cast<Hash>(hash), true, finished, maker);
    }

    // 受理時だけ状態を生成して候補を追加する、期待償却 O(log W + S)、到着キュー初回確保は O(W S)
    template<class Maker, class S>
    requires StepArg<S>
    bool push_lazy(Cost cost, S step, Maker&& maker) {
        return engine_.push_candidate(turn_, step, cost, now_hash_, false, false, std::forward<Maker>(maker));
    }

    // 受理時だけ状態を生成して候補を追加する、期待償却 O(log W + S)、到着キュー初回確保は O(W S)
    template<class Maker, class S, class B>
    requires StepArg<S> && BoolArg<B>
    bool push_lazy(Cost cost, S step, B finished, Maker&& maker) {
        return engine_.push_candidate(turn_, step, cost, now_hash_, false, finished, std::forward<Maker>(maker));
    }

    // 受理時だけ状態を生成して候補を追加する、期待償却 O(log W + S)、到着キュー初回確保は O(W S)
    template<class Maker, class H, class S>
    requires CompatibleHashArg<H, Hash> && StepArg<S>
    bool push_lazy(Cost cost, H hash, S step, Maker&& maker) {
        return engine_.push_candidate(turn_, step, cost, static_cast<Hash>(hash), true, false, std::forward<Maker>(maker));
    }

    // 受理時だけ状態を生成して候補を追加する、期待償却 O(log W + S)、到着キュー初回確保は O(W S)
    template<class Maker, class H, class S, class B>
    requires CompatibleHashArg<H, Hash> && StepArg<S> && BoolArg<B>
    bool push_lazy(Cost cost, H hash, S step, B finished, Maker&& maker) {
        return engine_.push_candidate(turn_, step, cost, static_cast<Hash>(hash), true, finished, std::forward<Maker>(maker));
    }
...
private:
    ...
    public:
        ...
        Engine(const Param& param, State&& initial, Cost cost, Hash hash, bool hashed) : p_(param) {
            ...
            p_.max_step = std::clamp(p_.max_step, 1, std::max(1, p_.max_turn));
        ...
        bool push_candidate(int from, int step, Cost cost, Hash hash, bool has_hash, bool finished, Maker&& maker) {
            ...
            if (!valid_cost(cost) || step <= 0 || step > p_.max_step || step > p_.max_turn - from) {
    ...
    private:
        ...
        void sample_time() {
            rt_.elapsed_us = multi_detail::now_us() - start_us_;
            unchecked_ = 0;
...
namespace {
using BS = cs::ChokudaiCopyMulti<>;
using Clock = cs::multi_detail::TestClock;
constexpr bool is_multi = true;
std::uint64_t checks = 0;
void check(bool ok, const char* expr, int line) {
    ++checks;
    if (!ok) {
        std::cerr << "FAIL multi:" << line << " " << expr << '\n';
        std::abort();
    }
}
...
struct Reference {
    ...
    bool offer(const State& state, int turn, const Edge& e) {
        ...
        int d = 1;
        d = std::clamp(p.max_step,1,std::max(1,p.max_turn));
...
void deterministic() {
    ...
    for(int id=1;id<=64;++id){
        while((cs::multi_detail::mix(hash)&15)!=0)++hash;
...
void state_and_numeric() {
    ...
    using F=cs::ChokudaiCopyMulti<double,std::uint8_t>;
...
void csv() {
    const auto path=std::filesystem::temp_directory_path()/"chokudai_multi_test.csv";
...
int main() {
    ...
    std::cout<<"PASS multi checks="<<checks<<'\n';
}
#undef CHECK
#endif
