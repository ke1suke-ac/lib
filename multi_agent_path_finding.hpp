...
/*
 * multi_agent_path_finding_v04: AHC向けの複数エージェント経路計画・搬送solver。
 *
...
private:
    void finalize_moves() const {
        if (moves_sorted) return;
...
namespace mapf_detail {

inline constexpr int inf_distance = std::numeric_limits<int>::max() / 4;
...
struct distance_table {
    ...
    bool compatible(const mapf_graph& g) const {
        return graph_identity == &g && vertex_count == g.size();
    }

    void build_row(const mapf_graph& g, int row, int target) {
    ...
    int get(int target, int vertex) const { return get_by_row(row_of(target), vertex); }
};
...
struct reservation_table {
    ...
    bool transition_is_free(int agent, int from, int to, int time) const {
        if (external != nullptr && external_time_offset + time < external->horizon &&
            external->move_is_blocked(external_time_offset + time, from, to)) {
            return false;
        }
        const int next_owner = at(time + 1, to);
        if (next_owner != -1 && next_owner != agent) return false;

        const int opposite_agent = at(time, to);
        const bool moving_owner = opposite_agent >= 0 || opposite_agent <= -3;
        if (moving_owner && opposite_agent != agent &&
            at(time + 1, from) == opposite_agent) {
            return false;
        }
        return true;
    }

    int earliest_goal_hold_time(int agent, int goal) const {
        int earliest = 0;
        for (int time = max_time; time >= 0; --time) {
            const int goal_owner = at(time, goal);
        ...
        if (external != nullptr) {
            for (int time = earliest; time < max_time; ++time) {
                const int global_time = external_time_offset + time;
                if (global_time >= external->horizon) break;
                if (external->move_is_blocked(global_time, goal, goal)) {
                    earliest = time + 1;
                }
            }
        }
    ...
    void release_path([[maybe_unused]] int agent,
                      std::span<const int> path) {
        assert(!path.empty());
        const int arrival = int(path.size()) - 1;
        for (int time = 1; time <= max_time; ++time) {
            const int vertex = time <= arrival ? path[time] : path.back();
            int& value = owner[std::size_t(time) * std::size_t(vertex_count) +
                               std::size_t(vertex)];
            assert(value == agent);
            value = -1;
        }
...
struct astar_workspace {
    ...
    static std::uint64_t open_key(int estimated_total, int secondary_cost) {
    ...
    static int secondary_cost(const open_node& node) {
        const std::uint32_t bits = std::uint32_t(node.key) ^ 0x80000000U;
    ...
    astar_outcome find_path(const mapf_graph& graph,
        ...
        const int goal_hold_time = reservations.earliest_goal_hold_time(agent, goal);
        ...
        while (!open_storage.empty()) {
            ...
            const int current_secondary = secondary_cost(current);
            ...
            auto relax = [&](int to) {
                if (!reservations.transition_is_free(agent, current.vertex, to, current.time)) {
                    return;
                }
...
inline void compute_costs(mapf_result& result) {
...
inline bool better_result(const mapf_result& lhs,
...
inline mapf_result plan_prioritized_order(const mapf_graph& graph,
    ...
    for (const int agent : order) {
        astar_outcome path = workspace.find_path(graph,
                                                 agent,
                                                 starts[agent],
                                                 goals[agent],
                                                 goal_rows[agent],
...
inline std::vector<int> make_prioritized_order(const mapf_graph& graph,
                                               std::span<const int> starts,
                                               std::span<const int> goals,
                                               std::span<const int> goal_rows,
                                               const distance_table& distance,
                                               std::span<const int> goal_blocking,
                                               int attempt,
                                               std::mt19937_64& rng) {
    const int agent_count = int(starts.size());
...
inline mapf_result solve_prioritized(const mapf_graph& graph,
                                     std::span<const int> starts,
                                     std::span<const int> goals,
                                     std::span<const int> goal_rows,
                                     const distance_table& distance,
                                     ...
                                     astar_workspace& workspace,
                                     reservation_table& reservations) {
    workspace.prepare(graph.size(), max_time);
    reservations.prepare(graph.size(), max_time);
    mapf_result best;
    long long total_expanded = 0;
    const int restarts = std::max(1, options.prioritized_restarts);
    const int agent_count = int(starts.size());
    ...
    for (int goal_agent = 0; goal_agent < agent_count; ++goal_agent) {
        for (int path_agent = 0; path_agent < agent_count; ++path_agent) {
            ...
            if (via_goal != inf_distance && goal_to_goal != inf_distance &&
    ...
    for (int attempt_index = 0; attempt_index < restarts; ++attempt_index) {
        ...
        std::vector<int> order = make_prioritized_order(graph,
                                                         starts,
                                                         goals,
                                                         goal_rows,
                                                         distance,
        ...
        int failed_agent = -1;
        mapf_result attempt = plan_prioritized_order(graph,
                                                      starts,
                                                      goals,
                                                      goal_rows,
                                                      distance,
        ...
        ++best.attempts;
        if (better_result(attempt, best, options.objective)) {
...
inline std::vector<int> choose_lns_agents(const mapf_graph& graph,
                                          const mapf_result& current,
                                          std::span<const int> starts,
                                          std::span<const int> goal_rows,
                                          const distance_table& distance,
                                          int requested_size,
                                          int iteration,
                                          std::mt19937_64& rng) {
    const int agent_count = int(current.paths.size());
    const int neighborhood_size = std::clamp(requested_size, 1, agent_count);
...
inline void improve_with_lns(const mapf_graph& graph,
                             std::span<const int> starts,
                             std::span<const int> goals,
                             std::span<const int> goal_rows,
                             const distance_table& distance,
    ...
    for (int iteration = 0; iteration < options.lns_iterations; ++iteration) {
        ...
        const std::vector<int> selected = choose_lns_agents(graph,
                                                             best,
                                                             starts,
                                                             goal_rows,
                                                             distance,
...
inline bool pibt_step(const mapf_graph& graph,
                      const distance_table& distance,
                      std::span<const int> goal_rows,
                      std::span<const int> goals,
                      ...
                      pibt_workspace& workspace,
                      std::mt19937_64& rng) {
    const int agent_count = int(positions.size());
    assert(priority_class.empty() || int(priority_class.size()) == agent_count);
    assert(int(workspace.occupant.size()) == graph.size());
    assert(int(workspace.order.size()) == agent_count);
    std::fill(workspace.occupant.begin(), workspace.occupant.end(), -1);
    for (int agent = 0; agent < agent_count; ++agent) {
        workspace.occupant[positions[agent]] = agent;
    }
    ...
    for (int agent = 0; agent < agent_count; ++agent) {
        ...
        workspace.random_key[agent] = rng();
    }
    std::sort(workspace.order.begin(), workspace.order.end(), [&](int lhs, int rhs) {
        ...
        if (lhs_distance != rhs_distance) return lhs_distance > rhs_distance;
        return workspace.random_key[lhs] < workspace.random_key[rhs];
    });

    std::fill(workspace.next_positions.begin(), workspace.next_positions.end(), -1);
    std::fill(workspace.next_occupied.begin(), workspace.next_occupied.end(), 0);
    std::fill(workspace.visiting.begin(), workspace.visiting.end(), 0);
    workspace.assigned.clear();

    auto rollback = [&](std::size_t marker) {
        while (workspace.assigned.size() > marker) {
            const int agent = workspace.assigned.back();
            workspace.assigned.pop_back();
            workspace.next_occupied[workspace.next_positions[agent]] = 0;
            workspace.next_positions[agent] = -1;
        }
    };

    auto decide = [&](auto&& self, int agent, int forbidden_vertex) -> bool {
        if (workspace.next_positions[agent] != -1) return true;
        if (workspace.visiting[agent]) return false;
        workspace.visiting[agent] = 1;

        auto& candidates = workspace.candidates[agent];
        candidates.clear();
        candidates.reserve(std::size_t(graph.degree(positions[agent])) + 1);
        auto add_candidate = [&](int vertex) {
            ...
            candidates.push_back({goal_distance, penalty, rng(), vertex});
        };
        ...
        // 必要になった候補だけ選ぶ。乱数キー自体は従来どおり先に全て生成する
        for (auto next = candidates.begin(); next != candidates.end(); ++next) {
            std::iter_swap(next, std::min_element(next, candidates.end(), less));
            const pibt_candidate& choice = *next;
            const int to = choice.vertex;
            if (to == forbidden_vertex || workspace.next_occupied[to]) continue;
            const std::size_t marker = workspace.assigned.size();
            workspace.next_positions[agent] = to;
            workspace.next_occupied[to] = 1;
            workspace.assigned.push_back(agent);

            const int blocking_agent = workspace.occupant[to];
            bool accepted = blocking_agent == -1 || blocking_agent == agent;
            if (!accepted && workspace.next_positions[blocking_agent] != -1) {
                accepted = workspace.next_positions[blocking_agent] != positions[agent];
            } else if (!accepted) {
                accepted = self(self, blocking_agent, positions[agent]);
            }
            if (accepted) {
                workspace.visiting[agent] = 0;
                return true;
            }
            rollback(marker);
        }
    ...
    for (const int agent : workspace.order) {
        if (workspace.next_positions[agent] != -1) continue;
        if (!decide(decide, agent, -1)) return false;
    }

    previous_positions = positions;
    positions.swap(workspace.next_positions);
    return true;
}

inline mapf_result solve_pibt(const mapf_graph& graph,
                              std::span<const int> starts,
                              std::span<const int> goals,
                              std::span<const int> goal_rows,
                              const distance_table& distance,
    ...
    mapf_result best;
    const int agent_count = int(starts.size());
    const int attempts = std::max(1, std::min(options.prioritized_restarts, 8));
    pibt_workspace workspace(graph.size(), agent_count);
...
// 同じグラフ上でsolve/repairを繰り返すための再利用workspace。
// graphはworkspaceの使用中に変更・破棄してはならない。
struct mapf_workspace {
    const mapf_graph* graph_identity = nullptr;
    mapf_detail::distance_table distance;
    mapf_detail::astar_workspace astar;
    mapf_detail::reservation_table reservations;
    ...
    mapf_workspace(mapf_workspace&&) = default;
    mapf_workspace& operator=(mapf_workspace&&) = default;

    void clear() {
        graph_identity = nullptr;
        distance = {};
        astar = {};
        reservations = {};
        goal_rows.clear();
    ...
    bool prepare(const mapf_graph& graph, std::span<const int> targets) {
        ...
        if (graph_identity != &graph || !distance.compatible(graph)) {
...
void assign_tasks_greedy(const distance_table& distance,
                         std::span<const mapd_task> tasks,
                         std::span<const int> positions,
                         std::span<const int> idle_agents,
                         std::span<const int> available_tasks,
                         std::vector<int>& assigned_task,
                         std::vector<unsigned char>& task_status,
                         int remaining_time = std::numeric_limits<int>::max());

void assign_tasks_greedy_separated(const distance_table& distance,
                                   std::span<const mapd_task> tasks,
                                   std::span<const int> positions,
                                   std::span<const int> idle_agents,
                                   std::span<const int> available_tasks,
                                   std::vector<unsigned char> endpoint_used,
                                   std::vector<int>& assigned_task,
                                   std::vector<unsigned char>& task_status,
                                   int remaining_time = std::numeric_limits<int>::max());

void assign_tasks_min_cost_flow(const distance_table& distance,
                                std::span<const mapd_task> tasks,
                                std::span<const int> positions,
                                std::span<const int> idle_agents,
                                std::span<const int> available_tasks,
...
inline int path_position(std::span<const int> path, int time) {
    return path[std::min(time, int(path.size()) - 1)];
}

inline bool validate_goal_endpoints(std::span<const int> starts,
                                     std::span<const int> goals,
                                     const mapf_result& result) {
    const int agent_count = int(starts.size());
...
inline bool validate_paths(const mapf_graph& graph,
                           std::span<const int> starts,
                           const std::vector<std::vector<int>>& paths,
                           const mapf_reservations* external = nullptr) {
    const int agent_count = int(starts.size());
    if (graph.size() <= 0 || int(paths.size()) != agent_count) return false;
    if (external != nullptr &&
...
inline mapf_result plan_selected_agents(
    const mapf_graph& graph,
    std::span<const int> starts,
    std::span<const int> goals,
    std::span<const int> goal_rows,
    const distance_table& distance,
    ...
    int max_time,
    const mapf_reservations* external,
    int external_time_offset,
    const mapf_options& options,
    const timer& deadline,
    std::mt19937_64& rng,
    ...
    std::stable_sort(base_order.begin(), base_order.end(), [&](int lhs, int rhs) {
        const int lhs_shortest = distance.get_by_row(goal_rows[lhs], starts[lhs]);
        const int rhs_shortest = distance.get_by_row(goal_rows[rhs], starts[rhs]);
    ...
    if (!reservations.reset(starts, external, external_time_offset)) return best;
    for (int agent = 0; agent < agent_count; ++agent) {
    ...
    for (int attempt = 0; attempt < restarts; ++attempt) {
        ...
        for (const int agent : order) {
            astar_outcome path = astar.find_path(graph,
                                                 agent,
                                                 starts[agent],
                                                 goals[agent],
                                                 goal_rows[agent],
        ...
        if (continue_search) {
            for (const int agent : planned_agents) {
                reservations.release_path(agent, candidate.paths[agent]);
            }
            reservations.visit_count = fixed_visit_count;
        }
        if (success && better_result(candidate, best, options.objective)) {
            const int attempts = best.attempts;
            best = std::move(candidate);
            best.attempts = attempts;
        }
...
// 搬送解の移動・衝突・pickup・delivery時刻を検証する。O((V + K)T + Q log Q)
inline bool validate_mapd_result(const mapf_graph& graph,
                                 std::span<const mapd_agent> agents,
                                 std::span<const mapd_task> tasks,
                                 const mapd_result& result) {
    ...
    if (int(result.paths.size()) != agent_count ||
...
// pickup-deliveryタスクを割り当て、衝突なしの搬送経路をPIBTで生成する。O(T(K log K + A))
inline mapd_result solve_mapd(const mapf_graph& graph,
                              std::span<const mapd_agent> agents,
                              std::span<const mapd_task> tasks,
                              const mapd_options& options) {
    ...
    mapf_detail::timer deadline(options.time_limit_ms, options.deadline);
    mapf_detail::distance_table distance(graph, targets);
    ...
    mapf_detail::pibt_workspace pibt_state(graph.size(), agent_count);
    ...
    for (int time = 0; time < options.max_time; ++time) {
        ...
        if (deadline.expired()) {
            for (int remaining = time; remaining < options.max_time; ++remaining) {
                for (int agent = 0; agent < agent_count; ++agent) {
        ...
        // 未解放taskも全て完了し、全員がparking上なら以後の位置は変わらない
        if (result.completed_tasks == task_count && positions == parking_goal) {
            for (int agent = 0; agent < agent_count; ++agent) {
        ...
        if (!idle_agents.empty() && !available_tasks.empty()) {
            if (assignment_strategy == mapd_assignment_strategy::greedy) {
                mapf_detail::assign_tasks_greedy(distance,
                                                  tasks,
                                                  positions,
                                                  idle_agents,
                                                  available_tasks,
                ...
                mapf_detail::assign_tasks_greedy_separated(
                    distance,
                    tasks,
                    positions,
                    idle_agents,
                    available_tasks,
                    std::move(endpoint_used),
                    assigned_task,
                    task_status,
                    options.max_time - time);
            } else {
                mapf_detail::assign_tasks_min_cost_flow(distance,
                                                        tasks,
                                                        positions,
                                                        idle_agents,
                                                        available_tasks,
        ...
        const bool moved = mapf_detail::pibt_step(graph,
                                                  distance,
                                                  goal_rows,
                                                  goals,
                                                  positions,
...
// 静的MAPF解の始点・終点・移動・頂点衝突・辺衝突を検証する。O((V + K)T)
inline bool validate_mapf_result(const mapf_graph& graph,
                                 std::span<const int> starts,
                                 std::span<const int> goals,
                                 const mapf_result& result) {
    return mapf_detail::validate_goal_endpoints(starts, goals, result) &&
           mapf_detail::validate_paths(graph, starts, result.paths);
}

// 外部時空間予約との衝突も含めて静的MAPF解を検証する。
inline bool validate_mapf_result(const mapf_graph& graph,
                                 std::span<const int> starts,
                                 std::span<const int> goals,
                                 const mapf_result& result,
                                 const mapf_reservations& external_reservations) {
    return mapf_detail::validate_goal_endpoints(starts, goals, result) &&
           mapf_detail::validate_paths(
               graph, starts, result.paths, &external_reservations);
}

// 複数エージェントの衝突なし経路をヒューリスティックに生成する。概ねO(R K H E log(VH))
inline mapf_result solve_mapf(const mapf_graph& graph,
                              std::span<const int> starts,
                              std::span<const int> goals,
                              const mapf_options& options) {
...
inline mapf_result solve_mapf(const mapf_graph& graph,
    ...
    const mapf_detail::distance_table& distance = workspace.distance;
    ...
    for (int agent = 0; agent < agent_count; ++agent) {
        ...
        if (shortest == mapf_detail::inf_distance) {
    ...
    if (options.strategy == mapf_strategy::prioritized) {
        result = mapf_detail::solve_prioritized(graph,
                                                starts,
                                                goals,
                                                workspace.goal_rows,
                                                distance,
        ...
        mapf_detail::improve_with_lns(graph,
                                      starts,
                                      goals,
                                      workspace.goal_rows,
                                      distance,
        ...
        result = mapf_detail::solve_pibt(graph,
                                         starts,
                                         goals,
                                         workspace.goal_rows,
                                         distance,
        ...
        if (!deadline.expired()) {
            ...
            consider_construction(mapf_detail::solve_pibt(graph,
                                                           starts,
                                                           goals,
                                                           workspace.goal_rows,
                                                           distance,
        ...
        mapf_result primary = mapf_detail::solve_prioritized(
            graph,
            starts,
            goals,
            workspace.goal_rows,
        ...
        if (!primary_succeeded && !deadline.expired()) {
            ...
            consider_construction(mapf_detail::solve_prioritized(
                graph,
                starts,
                goals,
                workspace.goal_rows,
...
// 既存の静的MAPF解をLNSで改善する。概ねO(I K H E log(VH))
inline mapf_result improve_mapf(const mapf_graph& graph,
                                std::span<const int> starts,
                                std::span<const int> goals,
                                const mapf_result& initial_solution,
...
inline mapf_result improve_mapf(const mapf_graph& graph,
    ...
    mapf_detail::compute_costs(unchanged);
...
inline mapf_result repair_mapf(const mapf_graph& graph,
    ...
    if (agent_count == 0 || int(goals.size()) != agent_count || graph.size() == 0 ||
        ...
        !mapf_detail::validate_paths(graph, starts, base.paths)) {
        return failure;
    }
    ...
    for (int agent = 0; agent < agent_count; ++agent) {
        if (starts[agent] < 0 || starts[agent] >= graph.size() ||
            goals[agent] < 0 || goals[agent] >= graph.size() ||
            start_owner[starts[agent]] != -1 || goal_owner[goals[agent]] != -1) {
            return failure;
        }
        start_owner[starts[agent]] = agent;
        goal_owner[goals[agent]] = agent;
    ...
    workspace.selected.assign(agent_count, 0);
    for (const int agent : request.agents) {
        if (agent < 0 || agent >= agent_count || workspace.selected[agent]) return failure;
        workspace.selected[agent] = 1;
    }
    ...
    auto choose_result = [&](mapf_result candidate, bool unchanged_valid) {
        ...
        if (unchanged_valid &&
            (!candidate.success ||
             mapf_detail::better_result(unchanged, candidate, options.objective))) {
    ...
    if (request.end_time < 0) {
        ...
        for (int agent = 0; agent < agent_count; ++agent) {
            if (!workspace.selected[agent] && base.paths[agent].back() != goals[agent]) {
                return failure;
            }
        }
        bool unchanged_valid = base_matches_goals;
        if (unchanged_valid && request.external_reservations != nullptr) {
            unchanged_valid = mapf_detail::validate_paths(
        ...
        mapf_result candidate = mapf_detail::plan_selected_agents(
            graph,
            starts,
            goals,
            workspace.goal_rows,
    ...
    for (int agent = 0; agent < agent_count; ++agent) {
        for (int time = 0; time <= window; ++time) {
            local_paths[agent][time] =
                mapf_detail::path_position(base.paths[agent], begin_time + time);
        }
    ...
    mapf_result local_candidate = mapf_detail::plan_selected_agents(
        graph,
        local_starts,
        local_goals,
        workspace.goal_rows,
    ...
    for (const int agent : request.agents) {
        ...
        for (int time = 0; time < begin_time; ++time) {
            path.push_back(mapf_detail::path_position(base.paths[agent], time));
        }
        for (int time = 0; time <= window; ++time) {
            path.push_back(mapf_detail::path_position(local_candidate.paths[agent], time));
        }
    ...
    candidate.success = true;
    candidate.status = mapf_status::success;
    mapf_detail::compute_costs(candidate);
    return choose_result(std::move(candidate), unchanged_valid);
}
...
inline int task_assignment_cost(const distance_table& distance,
                                int agent_vertex,
                                const mapd_task& task,
                                int remaining_time = std::numeric_limits<int>::max()) {
    const int to_pickup = distance.get(task.pickup, agent_vertex);
    const int to_delivery = distance.get(task.delivery, task.pickup);
    if (to_pickup == inf_distance || to_delivery == inf_distance) return inf_distance;
    // 衝突を無視した最短時間でも間に合わない組は割り当てない。
    if (to_pickup + to_delivery > remaining_time) return inf_distance;
    // 空荷の移動を重めに評価する。完了可能性の下界とは分けて扱う。
    return 2 * to_pickup + to_delivery;
}

inline void assign_tasks_greedy(const distance_table& distance,
                                std::span<const mapd_task> tasks,
                                std::span<const int> positions,
                                std::span<const int> idle_agents,
                                std::span<const int> available_tasks,
    ...
    for (const int agent : idle_agents) {
        ...
        for (const int task : available_tasks) {
            const int cost = task_assignment_cost(distance, positions[agent], tasks[task], remaining_time);
            if (cost != inf_distance) pairs.push_back({cost, agent, task});
        }
...
inline void assign_tasks_greedy_separated(const distance_table& distance,
                                          std::span<const mapd_task> tasks,
                                          std::span<const int> positions,
                                          std::span<const int> idle_agents,
                                          std::span<const int> available_tasks,
...
inline void assign_tasks_min_cost_flow(const distance_table& distance,
                                       std::span<const mapd_task> tasks,
                                       std::span<const int> positions,
                                       std::span<const int> idle_agents,
                                       std::span<const int> available_tasks,
                                       ...
                                       std::vector<unsigned char>& task_status,
                                       int remaining_time) {
    const int agent_count = int(idle_agents.size());
    const int task_count = int(available_tasks.size());
    const int source = 0;
    const int agent_base = 1;
    const int task_base = agent_base + agent_count;
    const int sink = task_base + task_count;
    ...
    assignment_edges.reserve(std::size_t(agent_count) * std::size_t(task_count));
    ...
    flow_graph.flow(source, sink, std::min(agent_count, task_count));
