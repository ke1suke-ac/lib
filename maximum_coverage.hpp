...
namespace coverage_solver_detail {
...
struct prepared_problem {
    ...
    problem_view view() const {
        return {target_count, candidate_count, target_weights, candidate_costs,
                candidate_weight_sums, candidate_offsets, candidate_targets,
                target_offsets, target_candidates, target_rarity, forced, forbidden, active};
    }
...
struct state {
    ...
    long long resource(int candidate, resource_mode mode) const {
    ...
    long long uncovered_rarity_gain(int candidate) const {
        long long result = 0;
        for (int index = problem->candidate_offsets[candidate];
             index < problem->candidate_offsets[candidate + 1]; ++index) {
            const int target = problem->candidate_targets[index];
            result += (cover_count[target] == 0) * problem->target_rarity[target];
        }
        return result;
    }
    ...
    long long swap_delta(int remove_candidate, int add_candidate) const {
        long long result = gain(add_candidate) - loss(remove_candidate);
        return result + swap_recovery(remove_candidate, add_candidate);
    }

    long long swap_recovery(int remove_candidate, int add_candidate) const {
        long long result = 0;
        int lhs = problem->candidate_offsets[remove_candidate];
        int rhs = problem->candidate_offsets[add_candidate];
        const int lhs_end = problem->candidate_offsets[remove_candidate + 1];
        const int rhs_end = problem->candidate_offsets[add_candidate + 1];

        // 削除候補だけが覆う共通対象は、追加候補によって直ちに再被覆されるため損失を戻す
        while (lhs < lhs_end && rhs < rhs_end) {
            const int lhs_target = problem->candidate_targets[lhs];
            const int rhs_target = problem->candidate_targets[rhs];
            if (lhs_target < rhs_target) {
                ++lhs;
            } else if (rhs_target < lhs_target) {
                ++rhs;
            } else {
                if (cover_count[lhs_target] == 1) result += problem->target_weights[lhs_target];
                ++lhs;
                ++rhs;
            }
        }
...
struct time_keeper {
    ...
    void count_iteration() { ++iterations; }
};
...
using priority_real = double;

struct greedy_node {
    priority_real priority = 0;
    long long gain = 0;
    int candidate = -1;
};
...
inline priority_real greedy_priority(long long gain, long long resource,
                                     greedy_metric metric, int factor_permille) {
    if (metric == greedy_metric::gain || resource == 0) {
        if (resource == 0 && gain > 0) return std::numeric_limits<priority_real>::infinity();
        return static_cast<priority_real>(gain) * static_cast<priority_real>(factor_permille);
    }
    return static_cast<priority_real>(gain) * static_cast<priority_real>(factor_permille) /
           static_cast<priority_real>(resource);
}

inline int noise_factor(std::uint64_t construction_seed, int candidate, int noise_permille) {
...
inline void add_forced_candidates(state& current) {
...
inline bool better_maximum(const state& lhs, const state& rhs, resource_mode mode) {
...
inline bool better_set_cover(const state& lhs, const state& rhs) {
...
inline void greedy_fill_maximum(state& current, long long limit, resource_mode mode,
    ...
    auto heap_push = [&](greedy_node node) {
        heap.push_back(node);
        if (heap_ready) std::push_heap(heap.begin(), heap.end(), heap_less);
    };

    // 利用可否は列挙側で確定済み。選択済み候補の未被覆利得は0なのでheapへ入らない
    auto push_candidate = [&](int candidate, long long gain) {
        const long long candidate_resource = current.resource(candidate, mode);
        ...
        heap_push({greedy_priority(gain, candidate_resource, metric, factor), gain, candidate});
    };
    ...
    while (!heap.empty()) {
        ...
        if (current.selected[candidate]) continue;
        const long long candidate_resource = current.resource(candidate, mode);
        ...
        if (gain <= 0) continue;
        const int factor = noise_factor(construction_seed, candidate, noise_permille);
        const priority_real priority = greedy_priority(gain, candidate_resource, metric, factor);
        if (!heap.empty() && priority < heap.front().priority) {
            heap_push({priority, gain, candidate});
            continue;
        }
...
inline bool greedy_fill_set_cover(state& current, std::uint64_t construction_seed,
                                  int noise_permille,
    ...
    auto push_candidate = [&](int candidate, long long gain) {
        ...
        const int factor = noise_factor(construction_seed, candidate, noise_permille);
        heap_push({greedy_priority(gain, problem.candidate_costs[candidate],
    ...
    if (use_sparse_initialization) {
        ...
        for (int candidate = 0; candidate < problem.candidate_count; ++candidate) {
            if (!problem.active[candidate] || current.selected[candidate]) continue;
            const long long gain = current.uncovered_rarity_gain(candidate);
            push_candidate(candidate, gain);
        }
    }
    std::make_heap(heap.begin(), heap.end(), heap_less);
...
inline void remove_redundant_maximum(state& current) {
    bool changed = true;
    while (changed) {
        changed = false;
        for (int position = static_cast<int>(current.selected_candidates.size()) - 1;
            ...
            if (current.problem->forced[candidate]) continue;
            if (current.loss(candidate) == 0) {
...
inline void remove_redundant_set_cover(state& current) {
    ...
    for (int candidate : order) {
        if (current.selected[candidate] && current.unique_target_count(candidate) == 0) {
            current.remove(candidate);
        }
    }
}

inline int sample_unselected_budget_candidate(const state& current,
                                               fast_random& random,
                                               long long budget) {
    int best_candidate = -1;
    priority_real best_priority = -1;
    ...
    for (int attempt = 0; attempt < sample_count; ++attempt) {
        ...
        const priority_real priority = greedy_priority(
...
inline bool attempt_budget_one_swap(state& current, long long budget,
                                    fast_random& random) {
    const int add_candidate = sample_unselected_budget_candidate(
        current, random, budget);
    if (add_candidate < 0) return false;

    // 空き資源へ正利得候補をそのまま追加できる場合はswapせず追加する
    if (current.problem->candidate_costs[add_candidate] <= budget - current.total_cost) {
        if (current.gain(add_candidate) <= 0) return false;
    ...
    for (int attempt = 0; attempt < attempts; ++attempt) {
        ...
        const long long delta = current.swap_delta(remove_candidate, add_candidate);
        if (delta > best_delta ||
...
inline int removable_count(const state& current) {
    int result = 0;
    for (int candidate : current.selected_candidates) {
        result += !current.problem->forced[candidate];
    }
    return result;
}

inline int choose_ruin_size(const state& current, fast_random& random) {
    const int count = removable_count(current);
    if (count == 0) return 0;
...
inline void choose_random_ruin(const state& current, int ruin_size,
                               fast_random& random, greedy_workspace& workspace,
                               std::vector<int>& result) {
    std::vector<int>& candidates = workspace.removable_candidates;
    candidates.clear();
    candidates.reserve(current.selected_candidates.size());
    for (int candidate : current.selected_candidates) {
...
inline int choose_set_cover_removal_candidate(const state& current, fast_random& random,
                                               int sample_count) {
    const int selected_count = static_cast<int>(current.selected_candidates.size());
    if (selected_count == 0) return -1;
    const int attempts = selected_count <= sample_count ? selected_count : sample_count;
    int best_candidate = -1;
    priority_real best_priority = std::numeric_limits<priority_real>::infinity();
    for (int attempt = 0; attempt < attempts; ++attempt) {
        const int candidate = selected_count <= sample_count
                                  ? current.selected_candidates[attempt]
                                  : current.selected_candidates[random.next_int(selected_count)];
        if (current.problem->forced[candidate] ||
            current.problem->candidate_costs[candidate] == 0) {
            continue;
        }
        const priority_real priority =
            static_cast<priority_real>(current.unique_target_count(candidate)) /
            static_cast<priority_real>(current.problem->candidate_costs[candidate]);
        if (priority < best_priority) {
            best_priority = priority;
            best_candidate = candidate;
        }
...
inline void choose_set_cover_low_loss_ruin(state& current, int ruin_size,
                                           fast_random& random, int sample_count,
                                           std::vector<int>& result) {
    result.clear();
    result.reserve(ruin_size);
    for (int index = 0; index < ruin_size; ++index) {
        const int candidate = choose_set_cover_removal_candidate(
            current, random, sample_count);
        if (candidate < 0) break;
        current.remove(candidate);
        result.push_back(candidate);
    }
    for (auto it = result.rbegin(); it != result.rend(); ++it) current.add(*it);
}

inline void choose_related_ruin(const state& current, int ruin_size,
                                fast_random& random, greedy_workspace& workspace,
                                std::vector<int>& result) {
    const problem_view& problem = *current.problem;
...
inline bool apply_cardinality_lns(state& current, int max_selected,
                                  fast_random& random, int ruin_kind,
                                  greedy_workspace& workspace) {
    const int ruin_size = choose_ruin_size(current, random);
    if (ruin_size == 0) return false;
...
inline coverage_solution make_solution(const state& current, long long iterations,
                                       bool feasible) {
...
inline state state_from_initial(const problem_view& problem,
...
inline coverage_solution solve_cardinality_impl(
    const problem_view& problem, int max_selected,
    const coverage_solver_options& options,
    const std::span<const int>* initial_candidates) {
    if (max_selected < 0) throw std::invalid_argument("上限は非負でなければならない");
    ...
    while (!timer.expired(true)) {
        for (int offset = 0; offset < 2 && !timer.expired(); ++offset) {
            ...
            apply_cardinality_lns(best, max_selected, random, kind, workspace);
            timer.count_iteration();
        }
        round = (round + 1) % 2;
    }
...
inline coverage_solution solve_budget_impl(
    ...
    for (int restart = 0; restart < random_restarts && !timer.expired(true); ++restart) {
        ...
        for (int attempt = 0; attempt < swap_attempts && !timer.expired(); ++attempt) {
            attempt_budget_one_swap(candidate, budget, random);
            timer.count_iteration();
        }
...
inline bool apply_set_cover_lns(state& current, fast_random& random,
                                int ruin_kind, greedy_workspace& workspace) {
    const int ruin_size = choose_ruin_size(current, random);
    if (ruin_size == 0) return false;
    ...
    if (ruin_kind == 0) {
        ...
        choose_set_cover_low_loss_ruin(current, ruin_size, random, 48, removed);
    } else {
        choose_related_ruin(current, ruin_size, random, workspace, removed);
    }
    if (removed.empty()) return false;
...
inline coverage_solution solve_set_cover_impl(
    ...
    while (!timer.expired(true)) {
        for (int offset = 0; offset < 3 && !timer.expired(); ++offset) {
            ...
            apply_set_cover_lns(best, random, kind, workspace);
            timer.count_iteration();
        }
        round = (round + 1) % 3;
    }
...
public:
    ...
    coverage_solution solve_budgeted_maximum_coverage(
        ...
        return coverage_solver_detail::solve_budget_impl(problem, budget, options, nullptr);
    }
    ...
    coverage_solution improve_budgeted_maximum_coverage(
        ...
        const coverage_subproblem& subproblem = {}) const {
        const auto problem = prepare(subproblem);
        return coverage_solver_detail::solve_budget_impl(problem, budget, options, &initial_candidates);
    }
    ...
    coverage_solution solve_minimum_set_cover(
        ...
        const coverage_subproblem& subproblem = {}) const {
        const auto problem = prepare(subproblem);
        return coverage_solver_detail::solve_set_cover_impl(problem, options, nullptr);
    }
    ...
    coverage_solution improve_minimum_set_cover(
        ...
        const coverage_subproblem& subproblem = {}) const {
        const auto problem = prepare(subproblem);
        return coverage_solver_detail::solve_set_cover_impl(problem, options, &initial_candidates);
    }
    ...
    coverage_evaluation evaluate(
        ...
        const coverage_subproblem& subproblem = {}) const {
        const auto problem = prepare(subproblem);
        const auto current = coverage_solver_detail::state_from_initial(problem, selected_candidates);
...
private:
    coverage_solver_detail::prepared_problem problem_;
    ...
    coverage_solver_detail::problem_view prepare(const coverage_subproblem& sub) const {
        auto view = problem_.view();
    ...
    coverage_solution cardinality(
        coverage_cardinality_bounds bounds, const std::span<const int>* initial,
        const coverage_solver_options& options, const coverage_subproblem& sub) const {
        using namespace coverage_solver_detail;
        ...
        const int available = static_cast<int>(std::count(problem.forbidden.begin(), problem.forbidden.end(), 0));
        if (available < bounds.min_selected) {
            const time_keeper validate_options(options);
            state base(problem);
            add_forced_candidates(base);
            return make_solution(base, 0, false);
        }
        auto result = solve_cardinality_impl(problem, bounds.max_selected, options, initial);
...
namespace coverage_solver_selftest {

using coverage_solver_detail::fast_random;

void require(bool condition, const std::string& label) {
    if (!condition) throw std::runtime_error("test failed: " + label);
}

long long evaluate_weight(const coverage_problem& problem,
...
long long evaluate_cost(const coverage_problem& problem,
                        const std::vector<int>& selected) {
    long long result = 0;
...
bool covers_all(const coverage_problem& problem, const std::vector<int>& selected) {
    std::vector<unsigned char> covered(problem.target_weights.size(), 0);
    for (int candidate : selected) {
        for (int target : problem.covered_targets[candidate]) covered[target] = 1;
    }
...
coverage_solver_options deterministic_options(std::uint64_t seed = 1) {
...
void test_forced_forbidden_and_edges() {
    coverage_problem problem;
...
exact_answers brute_force(const coverage_problem& problem, int max_selected,
                          long long budget) {
...
void test_random_against_exact() {
    ...
    for (int test = 0; test < 180; ++test) {
        ...
        const coverage_solver_detail::prepared_problem prepared(problem);
...
void test_improve_and_validation() {
    coverage_problem problem;
...
void validate_solution_structure(const coverage_problem& problem,
...
void test_selected_search_consistency() {
...
template<class F>
void expect_invalid(F&& function, const std::string& label) {
    bool threw = false;
    try { function(); } catch (const std::invalid_argument&) { threw = true; }
    require(threw, label);
}

void test_subproblem_boundaries() {
...
void test_random_state_differences() {
    ...
    for (int trial = 0; trial < 50; ++trial) {
        ...
        const coverage_solver_detail::prepared_problem stored(problem);
        const auto view = stored.view();
        coverage_solver_detail::state current(view);
        for (int step = 0; step < 400; ++step) {
            ...
            if (current.selected[a] && !current.selected[b]) {
                ...
                require(current.swap_delta(a, b) == evaluate_weight(problem, selected) - current.covered_weight,
                        "swap difference matches independent union");
            }
        }
    }
}
...
// 再監査で追加: 終了可能な部分問題、境界値、期限切れLNSの復元を個別に照合する
void test_review_regressions() {
    ...
    // 有効な状態を壊した直後にdeadlineが切れても、全状態と作業maskを元へ戻す
    coverage_problem rollback{{1, 2, 3}, {1, 1, 1, 2}, {{0}, {1}, {2}, {0, 1, 2}}, {}, {}};
    const coverage_solver_detail::prepared_problem stored(rollback);
    const auto view = stored.view();
    options.deadline = std::chrono::steady_clock::now() - std::chrono::seconds(1);
    coverage_solver_detail::time_keeper timer(options);
    coverage_solver_detail::greedy_workspace workspace(4, 3, timer);
    fast_random random(71963);
    for (int kind = 0; kind < 3; ++kind) {
        auto current = coverage_solver_detail::state_from_initial(view, std::array<int, 3>{0, 1, 2});
        workspace.stamp = std::numeric_limits<int>::max() - 1;
        require(!coverage_solver_detail::apply_set_cover_lns(current, random, kind, workspace),
                "expired set cover LNS rejected");
        const auto answer = coverage_solver_detail::make_solution(current, 0, true);
        require(answer.selected_candidates == std::vector<int>({0, 1, 2}), "expired cover membership restored");
        validate_solution_structure(rollback, answer);
        require(std::all_of(workspace.blocked_candidates.begin(), workspace.blocked_candidates.end(),
                            [](unsigned char value) { return value == 0; }), "blocked mask reset");
    }
    for (int kind = 0; kind < 2; ++kind) {
        auto current = coverage_solver_detail::state_from_initial(view, std::array<int, 3>{0, 1, 2});
        require(!coverage_solver_detail::apply_cardinality_lns(current, 3, random, kind, workspace),
                "expired cardinality LNS rejected");
        const auto answer = coverage_solver_detail::make_solution(current, 0, true);
        require(answer.selected_candidates == std::vector<int>({0, 1, 2}), "expired cardinality membership restored");
        validate_solution_structure(rollback, answer);
    }
...
int main() {
    try {
        coverage_solver_selftest::run_all_tests();
        std::cout << "maximum_coverage_solver_v06: all tests passed (400 subproblems, 20000 state operations, legacy tests)\n";
